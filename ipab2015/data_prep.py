...
label_dict = { 'Active':0, 'Inconclusive':1, 'Inactive':2}
labels = [label_dict[mol.GetProp('PUBCHEM_ACTIVITY_OUTCOME')] for mol in mols ]
feats = np.asarray(fps_arr, dtype=np.float32)
labels = np.asarray( labels, dtype=np.int64 ) 

train_feats, val_feats, train_labels, val_labels = train_test_split( feats, labels, test_size=0.2, random_state=10  )

train_obj = gzip.open('ipab_train.pickle.gz', 'wb')
val_obj = gzip.open('ipab_validation.pickle.gz', 'wb' )
